            ...
            msg.save((err) => {
              if (err) {
                ...
                doc.members.forEach(member => {
                  ...
                  if (msg.readRole !== 2 || member.role < 3 || member._id === payload.userId.toString()) {
      ...
      if (err) {
        ...
        Group.findOne({'_id': req.query.groupId}, (err, doc) => {
          if (err) {
            ...
            let messages
            doc.members.forEach(member => {
              if (member._id === payload.userId) {
                roleInGroup = member.role
              }
            })
            if (roleInGroup === 3) {
              messages = allMessages.filter(msg => (msg.readRole !== 2 || msg.from.toString() === payload.userId))
            } else {
              messages = allMessages
            }
            res.send({code: 200, result: messages, success: true})
          }
        })
      } else {
        res.send({code: 3, msg: '未知错误', success: false})
